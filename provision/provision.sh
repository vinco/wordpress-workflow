...
# Set the wordpress version you want to install from
# http://core.svn.wordpress.org/tags/
wordpress_version="3.8"

# The project's name
project_name="No tiene nada que ver"
...
if [[ $ping_result == *bytes?from* ]]; then
	...
	if [[ ! -d /home/vagrant/wordpress-default/wp-login.php ]]; then
		echo "Installing wordpress"
		svn checkout http://core.svn.wordpress.org/tags/${wordpress_version} /home/vagrant/wordpress-default
		cd /home/vagrant/wordpress-default
		echo "Configuring WordPress trunk..."
		wp core config --dbname=wordpress_default --dbuser=wp --dbpass=wp --quiet --extra-php <<PHP
define( 'WP_DEBUG', true );
PHP
		wp core install --url=local.wordpress.dev --quiet --title="${project_name}" --admin_name=admin --admin_email="admin@local.dev" --admin_password="password"
	fi
...
## Linkeamos las carpetas de themes y plugins
rm -rf /home/vagrant/wordpress-default/wp-content/themes
rm -rf /home/vagrant/wordpress-default/wp-content/plugins
ln -s /home/vagrant/themes /home/vagrant/wordpress-default/wp-content
